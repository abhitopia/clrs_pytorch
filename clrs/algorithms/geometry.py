...
from ..probing import Array, ProbesDict, Stage, mask_one, array

Out = Tuple[Any, ProbesDict]


def segments_intersect(probes: ProbesDict, xs: Array, ys: Array) -> Out:
  """Segment intersection."""
  ...
  probes.push(
      Stage.HINT,
      next_probe={
          'i': mask_one(0, xs.shape[0]),
          'j': mask_one(0, xs.shape[0]),
          'k': mask_one(0, xs.shape[0]),
          'dir': np.copy(dirs),
  ...
  probes.push(
      Stage.HINT,
      next_probe={
          'i': mask_one(2, xs.shape[0]),
          'j': mask_one(3, xs.shape[0]),
          'k': mask_one(0, xs.shape[0]),
          'dir': np.copy(dirs),
          'on_seg': np.copy(on_seg)
      })
  ...
  probes.push(
      ...
      next_probe={
          'i': mask_one(0, xs.shape[0]),
          'j': mask_one(1, xs.shape[0]),
          'k': mask_one(2, xs.shape[0]),
          'dir': np.copy(dirs),
          'on_seg': np.copy(on_seg)
      })
  ...
  probes.push(Stage.OUTPUT, next_probe={'intersect': ret})
  probes.finalize()

  return ret, probes


def graham_scan(probes: ProbesDict, xs: Array, ys: Array) -> Out:
  """Graham scan convex hull (Graham, 1972)."""
  ...
  probes.push(
      Stage.HINT,
      next_probe={
          'best': mask_one(0, xs.shape[0]),
          'atans': np.copy(atans),
          'in_hull_h': np.copy(in_hull),
          'stack_prev': np.copy(stack_prev),
          'last_stack': mask_one(0, xs.shape[0]),
          'i': mask_one(0, xs.shape[0]),
          'phase': mask_one(0, 5)
      })
  ...
  probes.push(
      Stage.HINT,
      next_probe={
          'best': mask_one(best, xs.shape[0]),
          'atans': np.copy(atans),
          'in_hull_h': np.copy(in_hull),
          'stack_prev': np.copy(stack_prev),
          'last_stack': mask_one(last_stack, xs.shape[0]),
          'i': mask_one(best, xs.shape[0]),
          'phase': mask_one(1, 5)
      })
      ...
      while counter_clockwise(xs, ys, stack_prev[last_stack], last_stack,
        ...
        probes.push(
            ...
            next_probe={
                ...
                'stack_prev': np.copy(stack_prev),
                'last_stack': mask_one(last_stack, xs.shape[0]),
                'i': mask_one(A_pos[ind[i]], xs.shape[0]),
                'phase': mask_one(3, 5)
            })
...
def jarvis_march(probes: ProbesDict, xs: Array, ys: Array) -> Out:
  """Jarvis' march convex hull (Jarvis, 1973)."""
  ...
  probes.push(
      ...
      next_probe={
          ...
          'in_hull_h': np.copy(in_hull),
          'best': mask_one(0, xs.shape[0]),
          'last_point': mask_one(0, xs.shape[0]),
          'endpoint': mask_one(0, xs.shape[0]),
          'i': mask_one(0, xs.shape[0]),
          'phase': mask_one(0, 2)
      })
  ...
  probes.push(
      ...
      next_probe={
          ...
          'in_hull_h': np.copy(in_hull),
          'best': mask_one(best, xs.shape[0]),
          'last_point': mask_one(last_point, xs.shape[0]),
          'endpoint': mask_one(endpoint, xs.shape[0]),
          'i': mask_one(0, xs.shape[0]),
          'phase': mask_one(1, 2)
      })
